  ...
  return <div className="container mx-auto px-4 py-[5px]">
      ...
      duration: 0.5
        ...
        <div className="space-y-8">
          {section.questions.map((question, index) => <QuestionItem key={index} question={question} questionIndex={index} onSelect={points => onOptionSelect(index, points)} selectedValue={answers[section.title]?.[index]} />)}
        </div>

        <div className="flex justify-between mt-12">
          {showPrevious ? <Button variant="outline" onClick={onPrevious} className="flex items-center gap-2">
              <ArrowLeftIcon size={16} />
              Précédent
            </Button> : <div />}
          
          {showNext && <Button onClick={onNext} className="flex items-center gap-2 bg-primary hover:bg-primary-hover">
              Suivant
